...
function renderMarkdown(markdown=''){
  ...
  html=html.replace(/\n{2,}/g,'</p><p>');
  html=html.replace(/\n/g,'<br>');
  if(!/^<(h3|h4|ul|pre|p)\b/.test(html))html=`<p>${html}</p>`;
...
async function checkAI(){
  ...
  try{
    ...
    showDiagnostic('The web UI is running, but the local API is unavailable. Start both services with “npm run dev” from the project root. The Vite and API ports may change automatically when their preferred ports are occupied.');
  }
}
